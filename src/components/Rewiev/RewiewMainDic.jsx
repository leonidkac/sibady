...
export default function RewievMainDic() {
  const { data } = useData();
  ...
  function getCountDic(data) {
    return data.reduce((acc, el) => {
      if (el.дисциплина && el.дисциплина.length > 3) {
        const dicSub = el.дисциплина.substring(3);
        acc[dicSub] = {
          name: dicSub,
  ...
  const filtredDic = data && data.rasp && Array.isArray(data.rasp) 
    ? data.rasp.filter(el => {
        if (el.дисциплина && el.дисциплина.length > 3) {
          const disSub = el.дисциплина.substring(3);
          return disSub === activeDic;
        }
  ...
  function handleActive(e) {
    ...
    setActiveDic(active); // Устанавливаем активный месяц
  }
